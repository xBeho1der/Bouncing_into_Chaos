...
from mars.configs.storage_configs.air import AirMachine
from mars.configs.storage_configs.base_storage_config import StorageConfig
from mars.configs.storage_configs.didi import DidiMachine
...
class MarsPipelineConfig(VanillaPipelineConfig):
    ...
    datamanager: DataManagerConfig = MarsDataManagerConfig()
    """specifies the datamanager config"""
    model: ModelConfig = ModelConfig()
    """specifies the model config"""
    storage_center: str = "none"
    """specifies the storage config, if not none, will overwrite the storage config according to the storage_center name"""
...
class MarsPipeline(Pipeline):
    ...
    def __init__(
        ...
        if config.storage_center != "none":
            storage_centers = {
                "didi": DidiMachine(),
                "air": AirMachine(),
            }
            assert (
                config.storage_center in storage_centers
            ), f"storage center {config.storage_center} not found."
            config.storage_config = storage_centers[config.storage_center]
        config.datamanager.dataparser.storage_config = config.storage_config
        ...
        self.config.model.max_num_obj = (
            self.config.datamanager.dataparser.max_input_objects
        )
        self.config.model.ray_add_input_rows = (
            self.config.datamanager.dataparser.add_input_rows
        )
        ...
        self._model = config.model.setup(
            ...
            car_latents=(
                self.datamanager.train_dataset.metadata["car_latents"]
                if use_car_latents
                else None
            ),
            car_nerf_state_dict_path=(
                self.datamanager.train_dataset.metadata["car_nerf_state_dict_path"]
                if use_car_latents
                else None
            ),
            use_depth=self.datamanager.train_dataset.use_depth,
            semantics_meta=(
                self.datamanager.train_dataset.metadata["semantics"]
    ...
    def get_average_eval_image_metrics(self, step: Optional[int] = None):
        ...
        with Progress(
            ...
            for (
                ...
                object_rays_info = self.datamanager.eval_dataset.metadata["obj_info"][
                    batch["image_idx"]
                ]

                object_rays_info_view = object_rays_info.expand(
                    camera_ray_bundle.shape[0], camera_ray_bundle.shape[1], -1, -1
                )

                camera_ray_bundle.metadata["object_rays_info"] = (
                    object_rays_info_view.reshape(
                        camera_ray_bundle.shape[0], camera_ray_bundle.shape[1], -1
                    ).detach()
                )
                # kplanes_camera = self.datamanager.eval_dataset.metadata[
                #     "kplanes_cameras"
                # ]
                # camera_ray_bundle.metadata[
                #     "kplanes_ray_bundle"
                # ] = kplanes_camera.generate_rays(
                #     camera_indices=batch["image_idx"],
                #     keep_shape=True,
                # )
    ...
    def load_pipeline(self, loaded_state: Dict[str, Any], step: int) -> None:
        ...
        try:
            ...
            print("Warning: strict=False when loading checkpoint")
