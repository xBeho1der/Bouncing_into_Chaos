...
def reflect(view_direction, normals):
    ...
    """
    return (
        2.0 * torch.sum(view_direction * normals, dim=-1, keepdims=True) * normals
        - view_direction
    )


def mae(weights, pred_normals, normals_gt):
    one_eps = 1 - torch.finfo(torch.float32).eps
    angle_diff = torch.arccos(
        torch.clamp(torch.sum(pred_normals * normals_gt, dim=-1), -one_eps, one_eps)
    )
    weighted_angle_diff = weights * angle_diff
    weighted_mae = weighted_angle_diff.sum() / weights.sum() * 180 / torch.pi
    return weighted_mae
...
class NeRF(torch.nn.Module):
    def __init__(
        ...
        self.views_linears = nn.ModuleList(
            [nn.Linear(input_ch_color_head + W, W // 2)]
            + [nn.Linear(W // 2, W // 2)] * (D // 2 - 1)
        )
    ...
    def forward(self, x):
        input_pts, input_views = torch.split(
            x, [self.input_ch, self.input_ch_color_head], dim=-1
        )
        h = input_pts
...
def plane_pts(rays, planes, id_planes, near, method="planes"):
    ...
    n_planes = (
        torch.ceil(torch.linalg.norm(plane_bds[:, -1] - plane_bds[:, 0]) / delta) + 1
    )
    ...
    if method == "planes":
        plane_id = torch.cat(
            [plane_id[:, :-1], n_planes.repeat_interleave(N_rays)[:, None]], dim=1
        )
    ...
    z_planes = (
        plane_normal[None, None, :] * torch.transpose(plane_id * delta, 0, 1)[..., None]
    )
...
def object2world(pts, dirs, pose, theta_y, dim=None, inverse=True):
    ...
    # Shift the object reference point to the middle of the bbox (vkitti2 specific)
    y_shift = torch.tensor([0.0, -1.0, 0.0]).unsqueeze(0).to(dim.device) * (
        dim[..., 1] / 2
    ).unsqueeze(-1)
...
def ray_box_intersection(ray_o, ray_d, aabb_min=None, aabb_max=None):
    ...
    intersection_map = torch.stack(
        list(torch.where(torch.logical_and(intersection_map, positive_far))), dim=1
    )
...
def box_pts(rays, pose, theta_y, dim=None, one_intersec_per_ray=False):
    ...
    if len(rays_o_o.shape) == 2:
        rays_o_o = rays_o_o.unsqueeze(1)
    if len(dirs_o.shape) == 2:
        dirs_o = dirs_o.unsqueeze(1)
    ...
    if z_ray_in_o is not None:
        ...
        pts_box_in_o = (
            ...
            + z_ray_in_o.unsqueeze(-1)
            * dirs_o[intersection_map[:, 0], intersection_map[:, 1], :]
        )
        ...
        z_vals_in_w = torch.linalg.norm(
            pts_box_in_w - rays_o_in_w, dim=1
        ) / torch.linalg.norm(rays_d_in_w, dim=-1)
        ...
        z_vals_out_w = torch.linalg.norm(
            pts_box_out_w - rays_o_in_w, dim=1
        ) / torch.linalg.norm(rays_d_in_w, dim=-1)

        # Get viewing directions for each ray-box intersection
        viewdirs_box_o = dirs_o[intersection_map[:, 0], intersection_map[:, 1], :]
        viewdirs_box_w = (
            1 / torch.linalg.norm(rays_d_in_w, dim=1)[:, None] * rays_d_in_w
        )
...
def get_closest_intersections(z_vals_w, intersection_map, N_rays, N_obj):
    ...
    z_scatterd_nz = torch.where(
        z_scatterd == 0, torch.ones_like(z_scatterd) * 1e10, z_scatterd
    )
    ...
    id_first_intersect = (
        id_z_vals[id_reduced[:, 0], id_reduced[:, 1]].reshape(-1).to(torch.int64)
    )
...
def combine_z(
    ...
    if z_vals_obj_w is None or z_vals_obj_w.shape == torch.Size([1]):
        ...
        z_vals_obj_w_sparse = torch.reshape(
            z_vals_obj_w_sparse, [N_rays, N_samples_obj * N_obj]
        )

    sample_range = torch.arange(0, N_rays).to(intersection_map.device)
    obj_range = (
        sample_range.unsqueeze(-1).unsqueeze(-1).repeat_interleave(N_obj, dim=1)
    ).repeat_interleave(N_samples_obj, dim=2)
    ...
    if z_vals_bckg is not None:
        ...
        z_vals, z_vals_indices = torch.sort(
            torch.cat((z_vals_obj_w_sparse, z_vals_bckg), dim=1), dim=1
        )

        bckg_range = (
            sample_range.unsqueeze(-1).unsqueeze(-1).repeat_interleave(N_samples, dim=1)
        )
        id_z_vals_bckg = torch.cat(
            (
                bckg_range,
                torch.searchsorted(z_vals, z_vals_bckg.contiguous()).unsqueeze(-1),
            ),
            dim=2,
        )
    ...
    id_z_vals_obj = torch.cat(
        [
            ...
            torch.reshape(
                torch.searchsorted(z_vals, z_vals_obj_w_sparse),
