...
class SemanticNerfWModel(Model):
    ...
    def __init__(
        self, config: SemanticNerfWModelConfig, object_meta: Dict, **kwargs
    ) -> None:
        assert "semantics" in object_meta.keys() and isinstance(
            object_meta["semantics"], Semantics
        )
    ...
    def populate_modules(self):
        ...
        self.field = NerfactoField(
            ...
            predict_reflectance=self.config.predict_reflectance,
        )
        ...
        if self.config.use_same_proposal_network:
            ...
            self.density_fns = [
                network.density_fn for _ in range(self.config.num_proposal_iterations)
            ]
        else:
        ...
        self.cross_entropy_loss = torch.nn.CrossEntropyLoss(
            reduction="mean", ignore_index=self.semantic_num
        )
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.config.predict_normals:
            normals = self.renderer_normals(
                field_outputs[FieldHeadNames.NORMALS], weights=weights
            )
