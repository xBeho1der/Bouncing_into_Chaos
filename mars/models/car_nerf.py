...
class CarNeRF(Model):
    ...
    def populate_modules(self):
        ...
        self.predict_normals = self.config.predict_normals
        self.predict_reflectance = self.config.predict_reflectance

        # whether to optimize the car_latents
        for idx, _ in self.car_latents.items():
            self.car_latents[idx] = nn.Parameter(
                self.car_latents[idx],
                requires_grad=self.training and self.config.optimize_latents,
            )

        self.fields = CarNeRF_Field(
            predict_normals=self.predict_normals,
            predict_reflectance=self.predict_reflectance,
        )
        if "car_nerf_state_dict_path" in self.kwargs:
            self.fields.load_state_dict(
                torch.load(self.kwargs["car_nerf_state_dict_path"]), strict=False
            )
        ...
        self.sampler_pdf = PDFSampler(
            num_samples=self.config.num_fine_samples, include_original=False
        )
    ...
    def inference_without_render(self, ray_bundle: RayBundle):
        ...
        weights_coarse_raw = torch.zeros(
            (*ray_bundle.shape, self.config.num_coarse_samples, 1),
            device=ray_bundle.camera_indices.device,
        )

        field_outputs_fine_raw = {
            FieldHeadNames.DENSITY: torch.zeros(
                (*ray_bundle.shape, self.config.num_fine_samples, 1),
                device=ray_bundle.camera_indices.device,
            ),
            FieldHeadNames.RGB: torch.zeros(
                (*ray_bundle.shape, self.config.num_fine_samples, 3),
                device=ray_bundle.camera_indices.device,
            ),
        ...
        for obj_id in unique_obj_ids:
            ...
            if self.predict_normals:
                with torch.enable_grad():
                    field_outputs_coarse = self.fields(
                        gaussian_samples.mean.view(1, -1, 3).requires_grad_(),
                        self.car_latents[int(obj_id)].view(1, -1).to(obj_id.device),
                        viewdirs=ray_samples_uniform.frustums.directions.reshape(
                            1, -1, 3
                        ),
                        covs=torch.diagonal(
                            gaussian_samples.cov, dim1=-2, dim2=-1
                        ).view(1, -1, 3),
                    )
            else:
                field_outputs_coarse = self.fields(
                    ...
                    covs=torch.diagonal(gaussian_samples.cov, dim1=-2, dim2=-1).view(
                        1, -1, 3
                    ),
                )

            iteration_keys = [FieldHeadNames.DENSITY, FieldHeadNames.RGB]
            if self.predict_normals:
                iteration_keys.append(FieldHeadNames.PRED_NORMALS)
                iteration_keys.append(FieldHeadNames.NORMALS)
            if self.predict_reflectance:
                iteration_keys.append("reflectance")

            for it in iteration_keys:
                field_outputs_coarse[it] = field_outputs_coarse[it].reshape(
                    (*gaussian_samples.mean.shape[:-1], -1)
                )
            ...
            ray_samples_pdf = self.sampler_pdf(
                ray_bundle[obj_msk], ray_samples_uniform, weights_coarse
            )
            ...
            if self.predict_normals:
                with torch.enable_grad():
                    field_outputs_fine = self.fields(
                        gaussian_samples_fine.mean.view(1, -1, 3).requires_grad_(),
                        self.car_latents[int(obj_id)].view(1, -1).to(obj_id.device),
                        viewdirs=ray_samples_pdf.frustums.directions.reshape(1, -1, 3),
                        covs=torch.diagonal(
                            gaussian_samples_fine.cov, dim1=-2, dim2=-1
                        ).view(1, -1, 3),
                    )
            else:
                field_outputs_fine = self.fields(
            ...
            for it in iteration_keys:
                field_outputs_fine[it] = field_outputs_fine[it].reshape(
                    (*gaussian_samples_fine.mean.shape[:-1], -1)
                )
        ...
        ray_samples_pdf_raw = self.sampler_pdf(
            ray_bundle, ray_samples_uniform_raw, weights_coarse_raw
        )
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        rgb_coarse_raw = torch.zeros(
            (*ray_bundle.shape, 3), device=ray_bundle.camera_indices.device
        )
        rgb_fine_raw = torch.zeros(
            (*ray_bundle.shape, 3), device=ray_bundle.camera_indices.device
        )
        accumulation_coarse_raw = torch.zeros(
            (*ray_bundle.shape, 1), device=ray_bundle.camera_indices.device
        )
        accumulation_fine_raw = torch.zeros(
            (*ray_bundle.shape, 1), device=ray_bundle.camera_indices.device
        )
        depth_coarse_raw = torch.zeros(
            (*ray_bundle.shape, 1), device=ray_bundle.camera_indices.device
        )
        depth_fine_raw = torch.zeros(
            (*ray_bundle.shape, 1), device=ray_bundle.camera_indices.device
        )
        ray_samples_uniform_raw = torch.zeros(
            (*ray_bundle.shape, self.config.num_coarse_samples),
            device=ray_bundle.camera_indices.device,
        )
        ray_samples_pdf_raw = torch.zeros(
            (*ray_bundle.shape, self.config.num_fine_samples),
            device=ray_bundle.camera_indices.device,
        )
        ...
        for obj_id in unique_obj_ids:
            ...
            field_outputs_fine = self.fields(
                ...
                self.car_latents[int(obj_id)].view(1, -1),
                viewdirs=ray_samples_pdf.frustums.directions.reshape(1, -1, 3),
                covs=torch.diagonal(gaussian_samples_fine.cov, dim1=-2, dim2=-1).view(
                    1, -1, 3
                ),
            )
