...
from mars.fields.kplanes_field import KPlanesDensityField, KPlanesField
...
class KPlanesModel(Model):
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if ray_bundle.times is not None:
            density_fns = [
                functools.partial(f, times=ray_bundle.times) for f in density_fns
            ]
        ...
        outputs = {
            "rgb_kplanes": rgb,
            "accumulation_kplanes": accumulation,
            "depth_kplanes": depth,
        }

        # These use a lot of GPU memory, so we avoid storing them for eval.
        if self.training:
            outputs["weights_list_kplanes"] = weights_list
            outputs["ray_samples_list_kplanes"] = ray_samples_list
    ...
    def get_metrics_dict(self, outputs, batch):
        ...
        # metrics_dict = {"psnr_kplanes": self.psnr(outputs["rgb_kplanes"], image)}
        metrics_dict = {}
        if self.training:
            metrics_dict["interlevel"] = interlevel_loss(
                outputs["weights_list_kplanes"], outputs["ray_samples_list_kplanes"]
            )
            ...
            if len(self.config.grid_base_resolution) == 4:
                ...
                metrics_dict["time_smoothness_proposal_net"] = time_smoothness(
                    prop_grids
                )
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        # loss_dict = {"rgbloss_kplanes": self.rgb_loss(image, outputs["rgb_kplanes"])}
        loss_dict = {}
        if self.training:
    ...
    def get_image_metrics_and_images(
        ...
        rgb = outputs["rgb_kplanes"]
        acc = colormaps.apply_colormap(outputs["accumulation_kplanes"])
        depth = colormaps.apply_depth_colormap(
            outputs["depth"], accumulation=outputs["accumulation_kplanes"]
        )

        combined_rgb = torch.cat([image, rgb], dim=0)
        combined_acc = torch.cat([acc], dim=0)
        combined_depth = torch.cat([depth], dim=0)
        ...
        metrics_dict = {
            "psnr_kplanes": float(self.psnr(image, rgb).item()),
            "ssim_kplanes": float(self.ssim(image, rgb)),
            "lpips_kplanes": float(self.lpips(image, rgb)),
        }
        metrics_dict = {}

        images_dict = {
            "img_kplanes": combined_rgb,
            "accumulation_kplanes": combined_acc,
            "depth_kplanes": combined_depth,
        }

        grid = self.field.grids[0]
        if len(grid.plane_coefs) > 3:
            ...
            for mat in [xt, yt, zt]:
                ...
                planes.append(
                    colormaps.apply_colormap(pca_plane.reshape(sh[0], sh[1], -1))
                )

            # obj_loc: [n_frames, n_max_objects, [x,y,z]]
            # obj_loc = self.metadata["obj_loc"]
            # offset = planes[0].shape[0]/2
            # ratio = planes[0].shape[0]/2

            # for i, frame in enumerate(obj_loc):
            #     if i >= len(obj_loc)/2:
            #         break
            #     for loc in frame:
            #         if loc[0] != -0.025 :
            ...
            #                 color = torch.zeros(3)
            #                 color[int(loc[3])%3] = 1
            #                 planes[0][int(offset+loc[0]*ratio), i] = color
            #                 planes[1][int(offset+loc[1]*ratio)-48, i] = color
            #                 planes[2][int(offset+loc[2]*ratio), i] = color
            #             except:
            #                 pass

            images_dict["xt"] = planes[0]
            images_dict["yt"] = planes[1]
            images_dict["zt"] = planes[2]

        for i in range(self.config.num_proposal_iterations):
            key = f"prop_depth_{i}"
            prop_depth_i = colormaps.apply_depth_colormap(
                outputs[key],
                accumulation=outputs["accumulation_kplanes"],
            )
