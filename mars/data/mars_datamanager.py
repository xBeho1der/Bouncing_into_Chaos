...
class MarsDataManager(VanillaDataManager):  # pylint: disable=abstract-method
    ...
    def create_train_dataset(self) -> MarsDataset:
        ...
        return MarsDataset(
            ...
            pred_normals=self.config.dataparser.pred_normals,
        )
    ...
    def next_train(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        # kplanes_ray_generator = RayGenerator(
        #     self.train_dataset.metadata["kplanes_cameras"].to(self.device),
        #     self.train_camera_optimizer,
        # )
        # kplanes_ray_bundle = kplanes_ray_generator(ray_indices)
        # ray_bundle.metadata["kplanes_ray_bundle"] = kplanes_ray_bundle
    ...
    def next_eval(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        ray_bundle.metadata["object_rays_info"] = object_rays_info.detach()
        # kplanes_ray_generator = RayGenerator(
        #     self.eval_dataset.metadata["kplanes_cameras"].to(self.device),
    ...
    def next_eval_image(self, step: int) -> Tuple[int, RayBundle, Dict]:
        for camera_ray_bundle, batch in self.eval_dataloader:
            ...
            # kplanes_cameras = self.eval_dataset.metadata["kplanes_cameras"]
            # kplanes_ray_bundle = kplanes_cameras.generate_rays(
            #     camera_indices=image_idx,
            #     keep_shape=True,
            # )
            # camera_ray_bundle.metadata["kplanes_ray_bundle"] = kplanes_ray_bundle
