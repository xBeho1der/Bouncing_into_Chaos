...
def _get_objects_by_frame(
    object_pose, object_meta, max_obj, n_cam, selected_frames, row_id
):
    """
                    ...
                    if selected_frames[0] <= obj_pose[0] <= selected_frames[1]:
                        if frame_objects:
                            if not all(frame_objects[-1][:2] == obj_pose[:2]):
                                max_in_frames = (
                                    len(frame_objects)
                                    if max_in_frames < len(frame_objects)
                                    else max_in_frames
                                )
...
def get_information(path):
    ...
    return np.array(
        [[_convert_to_float(j) for j in i.split(" ")] for i in c.splitlines()]
    )
...
class MarsVKittiDataParserConfig(DataParserConfig):
    ...
    _target: Type = field(default_factory=lambda: MarsVKittiDataparser)
    """target class to instantiate"""
    data: Path = Path("/data/vkitti/images/Scene06/clone")
    ...
    car_object_latents_path: Optional[Path] = Path(
        f"vkitti/latent_codes{sequence_id[-2:]}.pt"
    )
    """path of car object latent codes"""
    car_nerf_state_dict_path: Optional[Path] = Path("vkitti/epoch_805.ckpt")
    ...
    """whether to predict normals"""
    start_later: bool = False


@dataclass
class MarsVKittiDataparser(DataParser):
    ...
    def __init__(self, config: MarsVKittiDataParserConfig):
        ...
        self.semantic_path = (
            config.storage_config.datapath_dict["vKITTI-home"]
            / config.sequence_id
            / "clone"
            / "frames"
            / "classSegmentation"
        ...
        if "vKITTI-home" in self.config.storage_config.datapath_dict:
            self.data = (
                self.config.storage_config.datapath_dict["vKITTI-home"]
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        object_pose, object_meta, max_objects_per_frame, bboxes = _get_scene_objects(
            basedir
        )
        ...
        normal_dir = os.path.join(basedir, "frames/normal")
        # normal_dir = Path(
        #     f"/DATA_EDS2/chenjt2305/codes/DSINE/{self.config.sequence_id}"
        # )
        ...
        for camera in sorted(next(os.walk(rgb_dir))[1]):
            ...
            normal_frame_dir = os.path.join(normal_dir, camera)
            ...
            for frame in sorted(os.listdir(frame_dir)):
                if frame.endswith(".jpg"):
                    if len(frame.split("rgb_")) == 2:
                        frame_num_str = frame.split("rgb_")[1].split(".jpg")[0]
                    else:
                        frame_num_str = frame.split("rgb_")[0].split(".jpg")[0]
                    ...
                    if self.selected_frames[0] <= frame_num <= self.selected_frames[1]:
                        ...
                        if self.use_semantic:
                            ...
                            semantic_gt_name = os.path.join(
                                semantic_frame_dir, semantic_frame
                            )
                            semantic_name.append(semantic_gt_name)
                        if self.pred_normals:
                            normal_frame = "normal_" + frame_num_str + ".png"
                            normal_gt_name = os.path.join(
                                normal_frame_dir, normal_frame
                            )
                            normal_name.append(normal_gt_name)

                        ext = extrinsic[
                            frame_num * n_cam : frame_num * n_cam + n_cam, :
                        ][cam][2:]
        ...
        if self.use_object_properties or self.bckg_only:
            (
                ...
                scene_objects,
                scene_classes,
            ) = extract_object_information(self.config, visible_objects, object_meta)
