...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.max_pool2d(latents, kernel_size=latents.size()[2:])[
            :, :, 0, 0
        ]  # B x 512
...
class Decoder(nn.Module):
    def __init__(
        ...
        if predict_normals:
            ...
            self.field_head_pred_normals = PredNormalsFieldHead(
                in_dim=self.normals_mlp.get_out_dim()
            )
        if self.predict_reflectance:
            self.reflectance_mlp = MLP(
                in_dim=dim_embed + 1,
                num_layers=3,
                layer_width=64,
                out_dim=1,
    ...
    def switch_positional_encoding(self, mean, var=None, views=False):
        positional_encoding_fn = (
            self.viewdirs_encoding if views else self.positional_encoding
        )
    ...
    def forward(self, p_in, ray_d, latent, covs=None):
        ...
        if self.predict_normals:
            normals_inp = torch.cat(
                [p.reshape(N, -1), sigma_out.reshape(N, -1)], dim=-1
            )
            normals_out = self.normals_mlp(normals_inp).to(torch.float32)
            pred_normals = normals_out.view(feat_out.shape[1], -1).to(feat_out.device)
            pred_normals = self.field_head_pred_normals(pred_normals)
            normals = torch.autograd.grad(
                sigma_out,
                p_in,
                grad_outputs=torch.ones_like(sigma_out),
                retain_graph=True,
            )[0]
            normals = -torch.nn.functional.normalize(normals, dim=-1)
            output_dict[FieldHeadNames.NORMALS] = normals
            output_dict[FieldHeadNames.PRED_NORMALS] = pred_normals

        if self.predict_reflectance:
            reflectance = (
                self.reflectance_mlp(normals_inp)
                .view(feat_out.shape[1], -1)
                .to(torch.float32)
            )
            output_dict["reflectance"] = reflectance

        return feat_out, sigma_out, output_dict
...
class CarNeRF_Field(torch.nn.Module):
    ...
    def forward(self, xyz, latent, viewdirs=None, covs=None):
        ...
        rgb, sigma, output_dict = self.decoder(xyz, viewdirs, latent, covs)

        outputs = {
            FieldHeadNames.DENSITY: F.softplus(sigma),
            FieldHeadNames.RGB: torch.sigmoid(rgb),
        }
        if self.predict_normals:
            outputs[FieldHeadNames.NORMALS] = output_dict[FieldHeadNames.NORMALS]
            outputs[FieldHeadNames.PRED_NORMALS] = output_dict[
                FieldHeadNames.PRED_NORMALS
            ]
        if self.predict_reflectance:
            outputs["reflectance"] = output_dict["reflectance"]
        return outputs
